...
class Maverick(Client):
    def __init__(self, account_number, private_key, network, proxy=None):
        ...
        self.router_contract = self.get_contract(MAVERICK_CONTRACTS['router'], MAVERICK_ROUTER_ABI)
        self.pool_info_contract = self.get_contract(MAVERICK_CONTRACTS['pool_information'],
                                                    MAVERICK_POOL_INFORMATION_ABI)
        self.position_contract = self.get_contract(MAVERICK_CONTRACTS['position'], MAVERICK_POSITION_ABI)
        self.position_inspector_contract = self.get_contract(MAVERICK_CONTRACTS['position_inspector'],
                                                             MAVERICK_POSITION_INSPECTOR_ABI)
    ...
    async def swap(self):
        ...
        self.logger.info(f'{self.info} Swap on Maverick: {amount} {from_token_name} -> {to_token_name}')

        from_token_address, to_token_address = ZKSYNC_TOKENS[from_token_name], ZKSYNC_TOKENS[to_token_name]

        if from_token_name != 'ETH':
            await self.check_for_approved(from_token_address, MAVERICK_CONTRACTS['router'], amount_in_wei)
    ...
    async def add_liquidity(self):
        ...
        self.logger.info(f'{self.info} Add liquidity to Maverick USDC/ETH pool: {amount_from_settings} ETH')
    ...
    async def withdraw_liquidity(self):
        self.logger.info(f'{self.info} Withdraw liquidity from Maverick')
        ...
        if liquidity_balance != 0:
            ...
            if not await self.position_contract.functions.getApproved(position_id).call():
                ...
                await self.send_transaction(tx_approve_position_id)
