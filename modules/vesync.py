...
class VeSync(Client):
    def __init__(self, account_number, private_key, network, proxy=None):
        ...
        self.router_contract = self.get_contract(VESYNC_CONTRACTS['router'], VESYNC_ROUTER_ABI)
    ...
    async def swap(self):
        ...
        self.logger.info(f'{self.info} Swap on VeSync: {amount} {from_token_name} -> {to_token_name}')

        from_token_address, to_token_address = ZKSYNC_TOKENS[from_token_name], ZKSYNC_TOKENS[to_token_name]

        if from_token_name != 'ETH':
            await self.check_for_approved(from_token_address, VESYNC_CONTRACTS['router'], amount_in_wei)
