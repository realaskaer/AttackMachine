...
class Skydrome(DEX, Logger):
    def __init__(self, client):
        self.client = client
        Logger.__init__(self)
        self.network = self.client.network.name
        self.router_contract = self.client.get_contract(SKYDROME_CONTRACTS[self.network]['router'], SKYDROME_ROUTER_ABI)
    ...
    async def swap(self):
        ...
        self.logger_msg(*self.client.acc_info, msg=f'Swap on Skydrome: {amount} {from_token_name} -> {to_token_name}')
        ...
        min_amount_out, pool_stable_type = await self.get_out_data(from_token_address, to_token_address, amount_in_wei)

        await self.client.price_impact_defender(from_token_name, amount, to_token_name, min_amount_out)

        if from_token_name != 'ETH':
            await self.client.check_for_approved(
                from_token_address, SKYDROME_CONTRACTS[self.network]['router'], amount_in_wei
            )
