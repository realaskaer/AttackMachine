...
class Zebra(DEX, Logger):
    def __init__(self, client):
        self.client = client
        Logger.__init__(self)
        self.network = self.client.network.name
        self.router_contract = self.client.get_contract(ZEBRA_CONTRACTS[self.network]['router'], ZEBRA_ROUTER_ABI)
    ...
    async def swap(self):
        ...
        self.logger_msg(*self.client.acc_info, msg=f'Swap on Zebra: {amount} {from_token_name} -> {to_token_name}')
        ...
        if from_token_name != 'ETH':
            await self.client.check_for_approved(
                from_token_address, ZEBRA_CONTRACTS[self.network]['router'], amount_in_wei
            )
