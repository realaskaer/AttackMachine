...
## 🧩Модули
    ...
    56. Wrap/Unwrap ETH
    57. Wraps Abuser    (Делает врапы/анврапы ETH на агрегаторах)
    58. Merkly Attack   (Запускает refuel сразу в нескольких сетях)
    59. Inscription     (Минтит инскрипшены во всех сетях из LayerZero(которые реализованы в софте)
    60. Memcoin buyer   (Свапает ETH на указанный щиток)
    61. Colletor ETH    (Собирает весь баланс в ETH)
    62. Balancer ETH    (Уравнивает весь баланс в одинакове количество ETH)
    63. Stargate        (Делает свапы на Stargate во всех сетях для ETH, USDT, USDC)
    64. ScrollNFT       (Минтит NFT за деплой контрактов)
    65. Random Approve  (Делает случайный апрув на контракт DEX)
