...
---------------------------------------------OMNI-CHAIN CONTROL---------------------------------------------------------
    ...
    STG_STAKE_CONFIG | [Кол-во месяцев для лока, (минимальная, максимальная сумма для лока)].
    ...
    STARGATE_TOKENS | Выберите монеты, между которыми будут производиться свапы. Доступны: ETH, USDT, USDC, USDV, STG. 
        ...
        Также можно включить режим кругового бриджа. Пример: STARGATE_CHAINS = [1, (7, 22)]. Софт сделает бридж 
        ...
        Также можно сделать касание каждой сети указав STARGATE_CHAINS = [1, 7, 22] и L0_BRIDGE_COUNT равное количеству
        указанных сетей в STARGATE_CHAINS. Запустите модуль 1 раз и софт попытается сделать бридж из каждой указанной
